...
public class MainActivity extends BaseActivity {
    ...
    static void asyncFetchContactsFromServer() {
        HXSDKHelper.getInstance().asyncFetchContactsFromServer(new EMValueCallBack<List<String>>() {
            ...
            public void onSuccess(List<String> usernames) {
                ...
                ((DemoHXSDKHelper) HXSDKHelper.getInstance()).getUserProfileManager().asyncFetchContactInfosFromServer(
                        usernames, new EMValueCallBack<List<User>>() {
    ...
    public int getUnreadAddressCountTotal() {
        ...
        if (((DemoHXSDKHelper) HXSDKHelper.getInstance()).getContactList().get(Constant.NEW_FRIENDS_USERNAME) != null)
            unreadAddressCountTotal = ((DemoHXSDKHelper) HXSDKHelper.getInstance()).getContactList()
                ...
                public void run() {
                    try {
                        Toast.makeText(MainActivity.this,
                                MainActivity.this.getString(R.string.you_are_group) + ":" + _groupName, 1).show();
                        updateUnreadLabel();
                        if (currentTabIndex == 0)
                            chatHistoryFragment.refresh();
                        if (CommonUtils.getTopActivity(MainActivity.this).equals(GroupsActivity.class.getName())) {
